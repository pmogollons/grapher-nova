...
export default class extends Base {
  ...
  async fetchAsync(context) {
    ...
    if (this.isResolver) {
      ...
      if (this.cacher) {
        ...
        return await this.cacher.fetchAsync(cacheId, { query, context });
      }
  ...
  async getCountAsync(context) {
    ...
    if (this.cacher) {
      ...
      return this.cacher.fetchAsync(cacheId, { countCursor: count });
    }
  ...
  async _getCount() {
    ...
    if (body.$?.pipeline?.[0]?.$search) {
      const { $search } = body.$.pipeline[0];

      const result = await this.collection.aggregate([
        { $searchMeta: { ...$search, count: { type: "total" } } },
      ]);

      return result[0]?.count?.total ?? 0;
    }

    return await this.collection.find(body.$?.filters || {}, { fields: { _id: 1 } }).countAsync();
  }
