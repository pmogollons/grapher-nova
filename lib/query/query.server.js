...
export default class Query extends Base {
  ...
  async fetchAsync(context = {}) {
    ...
    const body = prepareForProcess(this.collection, { _id: true, ...this.body, $options: options }, this.params);
    let userId;
  ...
  async getCountAsync() {
    ...
    if (filters.$search) {
      ...
      return (await this.collection.aggregate(stages))[0]?.count?.total;
    }
